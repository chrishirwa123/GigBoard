...
// Get every accepted application (= an open chat thread) for this worker,
...
$list->execute([$worker_id]);

$conversations = $list->fetchAll(PDO::FETCH_ASSOC);


// ---------- Helpers (additive only — new page, nothing to preserve) ----------
...
@media (min-width:560px){
    ...
    .app{ min-height:calc(100vh - 56px); border-radius:24px; overflow:hidden; box-shadow:0 30px 80px -30px rgba(0,0,0,0.5); }
}
...
.convo-preview.unread-text{ color:var(--ink); font-weight:600; }
...
.empty svg{ width:52px;height:52px;color:var(--purple-400); margin-bottom:14px; }
.empty h3{ font-family:var(--font-display); color:var(--ink); font-size:17px; margin:0 0 6px; }
...
<div class="app">
    ...
    <div class="header">
        ...
        <div class="search-wrap" id="searchWrap">
            <input type="text" id="searchInput" placeholder="Search by name, job, or message…" autocomplete="off">
        </div>
    </div>
    ...
    <div class="list" id="list">
        ...
        <?php if(empty($conversations)): ?>
            ...
            <div class="empty">
                ...
                <p>Once an employer confirms you for a gig, your chat with them will show up here.</p>
            </div>
        ...
        <?php foreach($conversations as $c): ?>
            ...
            <?php
                $isUnread = $c['last_message'] !== null && (int)$c['last_sender_id'] !== (int)$worker_id;
                $preview = $c['last_message'] !== null ? gb_truncate($c['last_message']) : 'No messages yet — say hello';
                $searchBlob = strtolower($c['employer_name'] . ' ' . $c['job_title'] . ' ' . ($c['last_message'] ?? ''));
            ?>

            <a class="convo" href="chat.php?application_id=<?=(int)$c['application_id'];?>" data-search="<?=htmlspecialchars($searchBlob);?>">

                <div class="avatar" style="background:<?=gb_avatar_color($c['employer_id']);?>">
                    <?=htmlspecialchars(gb_initials($c['employer_name']));?>
                </div>

                <div class="convo-body">
                    <div class="convo-top">
                        <div class="convo-name">
                            <span class="name"><?=htmlspecialchars($c['employer_name']);?></span>
                            <?php if($isUnread): ?><span class="unread-dot"></span><?php endif; ?>
                        </div>
                        <span class="convo-time"><?=htmlspecialchars(gb_relative_time($c['last_message_at']));?></span>
                    </div>
                    <div class="convo-job"><?=htmlspecialchars($c['job_title']);?></div>
                    <div class="convo-preview<?php echo $isUnread ? ' unread-text' : ''; ?>"><?=htmlspecialchars($preview);?></div>
                </div>
    ...
    <button type="button" class="fab" id="fabBtn" aria-label="Scroll to top">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4"><line x1="12" y1="19" x2="12" y2="5"/><polyline points="5 12 12 5 19 12"/></svg>
    </button>
...
(function(){
    ...
    searchToggle.addEventListener('click', function(){
        ...
        else { searchInput.value = ''; filterList(''); }
    });
